...
def test_expand_skill_logits_preserves_native_column_alignment():
    """expand_skill_logits must NOT remap columns via skill_classes.

    The model is Avalanche's IncrementalClassifier: a single shared,
    growing head where column index already equals the global class
    id (train_on_class relies on this by computing cross_entropy
    against the real global label). A skill's raw logits are therefore
    already correctly aligned; expand_skill_logits should only pad out
    to output_dim, never reindex by the skill's owned-class set.

    This replaces a prior test that encoded the opposite (and wrong)
    assumption -- that a skill's raw output was a compact local head
    needing remapping via sorted(skill_classes). That assumption
    silently validated a bug that discarded a skill's real logit for
    its own class and substituted class 0's logit instead.
    """
    state = {"classifier.weight": torch.tensor([[1.0], [2.0], [3.0]])}
    # Raw logits are already indexed by global class id: columns 0, 1, 2.
    logits = torch.tensor([[3.0, 4.0, 9.0]])
    expanded = mod.expand_skill_logits(logits, state, {2}, output_dim=10)
    assert expanded.shape == (1, 10)
    # Native columns pass through untouched, regardless of skill_classes.
    assert expanded[0, 0].item() == 3.0
    assert expanded[0, 1].item() == 4.0
    assert expanded[0, 2].item() == 9.0
    # Columns beyond this skill's raw width are padded, not left at -inf
    # (a real -inf would make cross-entropy exactly +inf and break JSON
    # logging / running-average metrics).
    assert expanded[0, 9].item() == -1e4
    assert not torch.isneginf(expanded[0, 9])
