...
def test_best_match_selects_highest_score():
    ...
    def scorer(record, _query):
        score = 0.2 if record.name == "a" else 0.8
        return CompatibilityResult(score=score, accuracy=score)

    record, result = memory.best_match(None, scorer)
...
def test_best_match_returns_none_for_empty_memory():
    memory = SkillMemory(max_skills=2)
    record, result = memory.best_match(None, lambda *_: CompatibilityResult(1.0, 1.0))
    assert record is None
    assert result == CompatibilityResult(score=0.0, accuracy=0.0)
...
def test_optimizer_is_rebound_to_current_model_parameters():
    ...
    assert all(
        optimizer_param is model_param
        for optimizer_param, model_param in zip(optimizer_parameters, new_parameters)
    )
...
def test_clone_loads_skill_and_keeps_training_budget(monkeypatch):
    ...
    plugin = SkillMemoryPlugin(
        ...
        compatibility=lambda *_: CompatibilityResult(score=0.5, accuracy=0.5),
    )
...
def test_decision_audit_records_action_and_resource_usage(monkeypatch):
    ...
    assert event["memory_storage_bytes"] == sum(
